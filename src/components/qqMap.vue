...
export default {
  ...
  watch: {
    ...
    mapAll: function () {
      ...
      // 地图绘制
       try{
      TMap(vm.$store.state.mapKey).then(qq => {
        ...
        for (var i = 0; i < vm.path.length; i++) {
          ...
          for (var j = 0; j < vm.path[i].path.length; j++) {
            ...
            for (var t = 0; t < vm.path[i].path[j].length; t++) {
              var point = gcoord.transform(
                // [vm.path[i].path[j][t][1], vm.path[i].path[j][t][0]],
          ...
          //   var point = gcoord.transform(
          //     [vm.path[i].LatLng[f].lat, vm.path[i].LatLng[f].lng],
          //     gcoord.BMap, gcoord.GCJ02
          //   )
    ...
    funcShow (name, color, index) {
      ...
      TMap(vm.$store.state.mapKey).then(qq => {
        ...
        vm.map = new qq.maps.Map(document.getElementById('qQmap'), {
          // 地图的中心地理坐标。
          // center: new qq.maps.LatLng(39.908419, 116.397400),
          center: new qq.maps.LatLng( vm.mapAll.path[0].path[0][1],vm.mapAll.path[0].path[0][0]),
          zoom: 12
        })
        for (var i = 0; i < vm.path.length; i++) {
  ...
  mounted () {
    ...
    // var footerW = document.getElementById('map')
    // footerW.style.width = (document.body.clientWidth - 380) + 'px'
  }
}
</script>
