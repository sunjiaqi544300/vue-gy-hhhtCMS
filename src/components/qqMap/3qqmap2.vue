...
export default {
  name: "qqMap",
  props: ["path", "marker", "point", "alertObj","rightBottomData"],
  data() {
    return {
      color: {
        居住场所: "#f3282b",
    ...
    alertFn(evt) {
      ...
      this.info.setContent(
        `<div style="width:200px;" id="alertCont"  onclick="showMsg()" >
                          <h3 style="font-size:16px;">${vm.alertObj.shop_name}</h3>
                          <ul class="alertCont">
                              ...
                              <li style="display: flex;justify-content: space-between;background:#f5f6fa;padding:3px">
                                  <span style="font-size:13px;">防御/进攻品规1</span>
                                  <span style="font-size:13px;">${
                                    vm.alertObj.compproduct1 != ""
                                      ? vm.alertObj.compproduct1
                                      : "暂无"
                                  }</span>
                              </li>
                              <li style="display: flex;justify-content: space-between;padding:3px">
                                  <span style="font-size:13px;">防御/进攻品规2</span>
                                  <span style="font-size:13px;">${
                                    vm.alertObj.compproduct2 != ""
                                      ? vm.alertObj.compproduct2
                                      : "暂无"
                                  }</span>
                              </li>
          ...
          if (vm.path !== []) {
            if (vm.path != undefined) {
              for (let i = 0; i < vm.path.length; i++) {
                let innerPath = [];
                innerPath.color = vm.path[i].marketposition || "";
            ...
            if (vm.marker != undefined) {
              for (let i = 0; i < vm.marker.length; i++) {
                ...
                if (vm.marker[i].statedesc) {
                  ...
                  let icon;
                  if (typeof vm.marker[i].statedesc === "number") {
                    icon =
                      vm.marker[i].color === 1
                        ? icon1
                        : vm.marker[i].color === 2
                    ...
                    icon =
                      vm.marker[i].statedesc === "游击战"
                        ? icon4
                        : vm.marker[i].statedesc === "防御战"
                        ? icon3
                        : vm.marker[i].statedesc === "侧翼战"
                        ? icon1
                        : icon2;
                  }
