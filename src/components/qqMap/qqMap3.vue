<template>
  <div>
       <div class="qqMap" id="qqMap3" ref="qqMap"></div>
  </div>
</template>
...
export default {
  ...
  mounted: function() {
    ...
    document.getElementById("qqMap3").style.height = windowH + "px";
  ...
  methods: {
    alertFn(evt) {
      ...
      this.info.open();
        ...
        `<div style="width:200px;">
                          <h3 style="font-size:16px;">${evt.target.name != undefined ? evt.target.name : "暂时无法获取当前位置"}</h3>
                          <ul class="alertCont">
    ...
    drawMap() {
      ...
      TMap("H7KBZ-XMI66-4CXSJ-ERMEQ-UPCYZ-PABYG&libraries=convertor").then( qq => {
          vm.map = new qq.maps.Map(document.getElementById("qqMap3"), {
            // 地图的中心地理坐标。
