<template>
  <div class="qqMap4" id="qqMap4" ref="qqMap"></div>
</template>
...
export default {
  ...
  mounted: function() {
    ...
    document.getElementById("qqMap4").style.height = windowH + "px";
  ...
  methods: {
     ...
     this.info.open();
      ...
      this.info.setContent(
        `<div style="width:200px;position:relative;">
                          <h3 style="font-size:16px;">${vm.alertObj.biz_dist!= undefined ? vm.alertObj.biz_dist : "暂时无法获取当前位置"}</h3>
                          <ul class="alertCont">
                              ...
                              <li style="display: flex;justify-content: space-between;padding:3px">
                                  <span style="font-size:13px;">平均档位</span>
                                  <span style="font-size:13px;">${
                                    vm.alertObj.avglevel != ""
                                      ? vm.alertObj.avglevel
                                      : "暂无"
                                  }</span>
                              </li>
                              <li style="display: flex;justify-content: space-between;background:#f5f6fa;padding:3px">
                                  <span style="font-size:13px;">实际单箱结构</span>
                                  <span style="font-size:13px;">${
                                    vm.alertObj.avgstrip != ""
                                      ? vm.alertObj.avgstrip
                                      : "暂无"
                                  }万元/箱</span>
                              </li>
                               <li style="display: flex;justify-content: space-between;background:#f5f6fa;padding:3px">
                                  <span style="font-size:13px;">${vm.alertObj.prolistname}进货量</span>
                                  <span style="font-size:13px;">${
                                    vm.alertObj.prolist != ""
                                      ? vm.alertObj.prolist
                                      : vm.alertObj.prolist
                               ...
                               <li style="display: flex;justify-content: space-between;background:#f5f6fa;padding:3px">
                                  <span style="font-size:13px;">${vm.alertObj.competingname}进货量</span>
                                  <span style="font-size:13px;">${
                                    vm.alertObj.competing != ""
                                      ? vm.alertObj.competing
                                      : vm.alertObj.competing
                                  }条</span>
                              </li>
    ...
    this.info.setZIndex(99999)
    },
    drawMap() {
      let vm = this;
      TMap("H7KBZ-XMI66-4CXSJ-ERMEQ-UPCYZ-PABYG&libraries=convertor").then( qq => {
          vm.map = new qq.maps.Map(document.getElementById("qqMap4"), {
            // 地图的中心地理坐标。
          ...
          if (vm.path !== []) {
            if (vm.path != undefined) {
              for (let i = 0; i < vm.path.length; i++) {
                let innerPath = [];
                innerPath.color = vm.path[i].color || "";
