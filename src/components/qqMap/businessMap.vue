      ...
      TMap("H7KBZ-XMI66-4CXSJ-ERMEQ-UPCYZ-PABYG&libraries=convertor").then( qq => {
            ...
            if (vm.marker != undefined) {
              ...
              for (let i = 0; i < vm.marker.length; i++) {
                ...
                if (vm.marker[i]) {
                  ...
                  // if (typeof vm.marker[i].state === "number") {
                  //   icon =
                  //     vm.marker[i].color === 1
                  //       ? icon1
                  //       : vm.marker[i].color === 2
                  //       ? icon2
                  //       : icon3;
                  // } else if (typeof vm.marker[i].state === "string") {
                  //   icon =
                  //     vm.marker[i].state === "监管户"
                  //       ? icon1
                  //       : vm.marker[i].state === "连锁户"
                  //       ? icon2
                  //       : vm.marker[i].state === "潜力户"
                  //       ? icon5
                  //       : vm.marker[i].state === "核心价值户"
                  //       ? icon3
                  //       :icon4
                  // }
                  // console.log(vm.marker)
                  var marker = new qq.maps.Marker({
                    icon: vm.img.red, // vm.marker[i].color === 1?icon1:vm.marker[i].color === 2?icon2:icon3,
                    map: vm.map,
                    position: latLng,
          ...
          if (vm.point != undefined) {
            if (vm.point.length !== 0) {
              // let result = [vm.point[0],vm.point[1]];
              vm.map.panTo(new qq.maps.LatLng(vm.point[0],vm.point[1]));
              vm.map.zoomTo(11);
