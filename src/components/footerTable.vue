<template>
  <div id="footer" class="footer">
    <div class="footer_top">
      <div class="left">
        <el-input class="file_search" placeholder="请输入内容" v-model="search" size="mini">
      ...
      <div class="left button">
        ...
        <!--<el-button type="success" @click="handleDownloadAll()" size="small">导出所有数据</el-button>-->
        <el-button type="success" size="small" v-if="$route.query.id==='old'" @click="ENverificationToggle">验证
        </el-button>
      </div>
      <div class="right">
        <span class="data_num" style='margin-right:7px;color:black'>共有<i
            style='color:#5592F7'>{{table.length}}</i>条数据</span>
    ...
    <el-table id="tableW" border :data="table.slice((page-1)*200,(page-1)*200 + 200)" height="260"
      :row-class-name="tableRowClassName" :header-cell-style="headerStyle" @row-click="clickMapS" @select="selectDown"
      @select-all="selectDown">
      <el-table-column type="index" align="center" :index="(page-1)*200+1" label="序号" width="80">

      </el-table-column>
      <el-table-column v-for="(item,index) in tableName" align="center" :prop="item.key" :label="item.name"
    ...
    <transition name="el-zoom-in-bottom">
      <div class="tableAll" v-show="downDataTable">
        <div class="footer_top">
          <div class="left">
            <el-input class="file_search" placeholder="请输入内容" v-model="search">
          ...
          <div class="right">
            <span class="data_num" style='margin-right:7px;color:black'>共有<i
                style='color:#5592F7'>{{tableDownNumAll.length}}</i>条数据</span>
            <i class="el-icon-close close" @click="downDataTable = false, tableData = []"></i>
          </div>
        </div>
        <el-table id="tableAll_install" stripe border ref="multipleTable" :data="downTable" height="500"
          show-overflow-tooltip="true" @select="selectDown" @select-all="selectDownAll">
          <el-table-column type="selection" width="55">
          </el-table-column>
          <el-table-column type="index" :index="(DownPage-1)*200+1" label="序号" width="80">

          </el-table-column>
          <el-table-column v-for="(item,index) in tableName" :prop="item.key" :label="item.name" :width="item.width"
            :key="index">
